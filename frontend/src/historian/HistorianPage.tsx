...
export function HistorianPage({ patientId = "P-1042" }: { patientId?: string }) {
  ...
  return (
    <main className="historian-shell">
      ...
      <section className="historian-rules"><details><summary>Research rules and explanation</summary><p className="research-disclaimer">These configurable deltas are research-prototype explanations, not validated clinical weights or medical advice.</p><div className="rule-list">{record.rule_evaluations.map((rule) => <RuleRow key={rule.rule_key} rule={rule} />)}</div></details></section>
