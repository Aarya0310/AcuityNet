...
// `@.github/gsd-core/workflows/<stem>.md` include: line-start `@`, exact
 ...
 * import, which for a global skill is `.github/skills/gsd-<stem>/` — not
 * the project's working directory. `@./.github/...` would therefore always
...
function resolveSpecRootReference(body) {
    if (typeof body !== 'string' || body.length === 0)
        return body;
    if (!body.includes('@.github/gsd-core/workflows/'))
        return body;
    ...
    return body.replace(WORKFLOW_SPEC_ROOT_INCLUDE_RE, (match, stem, cr, offset) => {
        ...
        return (`To load this command's workflow spec: check for ` +
            `\`.github/gsd-core/workflows/${stem}.md\` relative to the current working ` +
            `directory first (project-local); if it is not there, fall back to ` +
            `\`.github/gsd-core/workflows/${stem}.md\` (the global install). If ` +
...
// `@.github/gsd-core/commands/gsd/<stem>.md` reference at a file that does
...
function convertClaudeToTraeMarkdown(content) {
    ...
    // converter — for `.github/`, `.github/`, AND `./.github/` alike —
    ...
    // left the doubled-prefix defect live for the `.github/` and
    // `.github/` forms specifically (found the same way, one regression-test
    // run later). Scoped to a `.trae/` tail so it cannot also swallow the
    // unprefixed `./copilot-instructions.md` form the very next pattern handles differently
 ...
 *   copilot-instructions.md    -> brandingRewrites['copilot-instructions.md']
 *   Claude Code  -> brandingRewrites['Claude Code']  (word-boundary, \bClaude Code\b)
 *   .github/     -> brandingRewrites['.github/']
 *
 ...
 * context-sensitive correction layered on top of the blanket `.github/` /
 * `.github/` -> pathPrefix substitution every the agent emit path
 ...
 * `node ".github/gsd-core/bin/gsd-tools.cjs"`) need it, since `~` does
 ...
 * every `@.github/gsd-core/…` include in a global install's workflows/
 ...
 * literal `.github/` segment, so it silently no-opped for any global install
 * under a non-default `--config-dir` (e.g. `.github-work`) — reproducing
 ...
 * copilot-instructions.md" (e.g. `See @README for project overview`), so anchoring to
 ...
 * like `echo "@.github/x"`, where rewriting `$HOME` to `~` inside
 ...
 * unconditionally rewrites any bare (non-`@`-prefixed) `.github/` substring
 * in the body to the computed pathPrefix (`.github/` for a global
 * install) and restores ONLY the `@`-prefixed form back to `~`
 * (`@.github/` → `@.github/`). `resolveSpecRootReference`'s
 * replacement text is deliberately imperative prose containing a literal,
 * non-`@`-prefixed `.github/gsd-core/workflows/<stem>.md` — running it
 ...
 * `@.github/gsd-core/workflows/<stem>.md` include line (which survives the
